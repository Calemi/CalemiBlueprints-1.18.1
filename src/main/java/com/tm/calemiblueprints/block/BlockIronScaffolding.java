...
public class BlockIronScaffolding extends Block {
    ...
    public void attack(BlockState state, Level world, BlockPos pos, Player player) {
        ...
        if (!player.isCrouching()) {
            BlockHelper.placeBlockRow(player, this, pos, DirectionHelper.getPlayerHorizontalDirection(player), 256);
        }

        //If the Player is not crouching, then place the block downwards.
        else {
            BlockHelper.placeBlockRow(player, this, pos, Direction.DOWN, 256);
        }
    }
    ...
    public void playerWillDestroy (Level level, BlockPos pos, BlockState state, Player player) {
        ...
        if (player.isCrouching()) {
            ...
            BlockScanner scanner = new BlockScanner(origin, defaultBlockState(), CBConfig.server.blockScannerMaxSize.get(), true);
